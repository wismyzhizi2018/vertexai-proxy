# VertexAI-Proxy: OpenClaw Gemini 接入指南

> [OpenClaw 是什么？访问地址](https://github.com/openclaw/openclaw)

把 Google Vertex AI 上的 Gemini 模型封装成 **OpenAI 兼容接口**，供 OpenClaw 及其他客户端使用。
...
| 鉴权错误 | 确认执行过 `gcloud auth application-default login`，项目已启用 `aiplatform.googleapis.com` |
